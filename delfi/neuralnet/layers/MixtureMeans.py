...
class MixtureMeansLayer(lasagne.layers.Layer):
    ...
    def get_output_for(self, input, deterministic=False, **kwargs):
        ...
        if not self.svi or deterministic:
            return [tt.dot(input, mW) + mb for mW, mb in zip(self.mWs, self.mbs)]
        else:
            uas = [self._srng.normal((input.shape[0], self.n_dim), dtype=dtype) for i in range(self.n_components)]
            mas = [tt.dot(input, mWm) + mbm for mWm, mbm in zip(self.mWs, self.mbs)]
