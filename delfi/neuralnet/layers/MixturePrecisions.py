...
class MixturePrecisionsLayer(lasagne.layers.Layer):
    def __init__(self, incoming, n_components, n_dim, svi=True,
                 mWs_init=linit.HeNormal(), mbs_init=linit.Constant([0.]),
                 sWs_init=linit.Constant([-5.]), sbs_init=linit.Constant([-5.]),
                 **kwargs):
    ...
    def get_output_for(self, input, deterministic=False, **kwargs):
        ...
        offdiag_mask = np.ones(self.n_dim, dtype=dtype) - np.eye(self.n_dim, dtype=dtype)

        if not self.svi or deterministic:
            zas_reshaped = [tt.reshape(tt.dot(input, mW) + mb, [-1, self.n_dim, self.n_dim]) for mW, mb in zip(self.mWs, self.mbs)]
        else:
            uas = [self._srng.normal((input.shape[0], self.n_dim**2), dtype=dtype) for i in range(self.n_components)]
            mas = [tt.dot(input, mW) + mb for mW, mb in zip(self.mWs, self.mbs)]
            sas = [tt.dot(input**2, tt.exp(2*sW)) + tt.exp(2*sb) for sW, sb in zip(self.sWs, self.sbs)]
            zas = [tt.sqrt(sa) * ua + ma for sa, ua, ma in zip(sas, uas, mas)]

            zas_reshaped = [tt.reshape(za, [-1, self.n_dim, self.n_dim]) for za in zas]
